...
import top.soaringlab.MTCICEP.condition.RelativeCondition;
import top.soaringlab.MTCICEP.event.IntervalEvent;
import top.soaringlab.MTCICEP.event.RawEvent;
import top.soaringlab.MTCICEP.mapper.TupleToIntervalMapper;
import top.soaringlab.MTCICEP.processor.D2IAHomogeneousIntervalProcessorFunction;
    ...
    public DataStream<W> runWithSQL(StreamExecutionEnvironment env) throws Exception {
        ...
        if (env.getStreamTimeCharacteristic() == TimeCharacteristic.EventTime) {
            tableEnv.registerDataStream("RawEvents",
                    keyedStream.map((MapFunction<S, Tuple4<String, Double, Long, String>>) event -> new Tuple4<>(event.getKey(), event.getValue(), event.getTimestamp(), event.getTimeScale())).returns(inputTupleInfoEventTime),
                    "ID, val, eventTime.rowtime, eventTime.timescale"
            );
        }
        else
        {
            tableEnv.registerDataStream("RawEvents",
                    keyedStream.map((MapFunction<S, Tuple3<String, Double, String>>) event -> new Tuple3<>(event.getKey(), event.getValue(), event.getTimeScale())).returns(inputTupleInfoProcessingTime),
                    "ID, val, eventTime.proctime, eventTime.timescale"
            );
        }
        ...
        TupleTypeInfo<Tuple7<String, Timestamp, Timestamp, Double, String, Integer, String>> tupleTypeInterval = new TupleTypeInfo<>(
                Types.STRING(),
                Types.SQL_TIMESTAMP(),
                Types.SQL_TIMESTAMP(),
                Types.DOUBLE(),
        ...
        DataStream<Tuple7<String, Timestamp, Timestamp, Double, String, Integer, String>> queryResultAsStream = tableEnv.toAppendStream(intervalResult, tupleTypeInterval);
