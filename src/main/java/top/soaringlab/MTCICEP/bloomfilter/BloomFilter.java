package top.soaringlab.MTCICEP.bloomfilter;
...
public class BloomFilter<E> implements Serializable {
    ...
    public String bitToString() {
        ...
        char currentCharState = '9';
        long currentCharTimes = 0;

        // 迭代 BitSet 并构建二进制字符串
        for (int i = 0; i < this.bitset.length(); ++i) {
            char now = bitset.get(i) ? 'a' : 'b';
            if (now == currentCharState) currentCharTimes+=1;
            else {
                if (currentCharTimes != 0) {
                    binaryRepresentation.append(currentCharState);
                    binaryRepresentation.append(currentCharTimes);
                }
                currentCharState = now;
                currentCharTimes = 1;
            }
        }
        if (currentCharTimes != 0) {
