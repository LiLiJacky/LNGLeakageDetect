...
//            System.out.println("Flushing incomplete interval for key: "+s + " due to end of stream signal");
        ...
        String tc = elements.get(0).getTimeScale();
        end = elements.get(elements.size() - 1).getTimestamp();
        I element;
        if (out != null)
            element = out.getDeclaredConstructor(long.class, long.class, double.class, String.class, String.class, String.class).newInstance(start, end, outputValue, outValueDescription, s, tc);
        else
            element = (I) new IntervalEvent(start, end, outputValue, outValueDescription, s, tc);
